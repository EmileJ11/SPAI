classdef PCOT < handle
    %PCOT Point Cloud Optimal Transport
    ...
    properties
        ...
        C
        Cx
        xi % expected distance (m) of transported image sources
    ...
    methods
        function obj = PCOT(PC1, PC2, xi)
            ...
            for ii = 1:PC1.n
                for jj = 1:PC2.n
                    C(ii,jj) = norm(PC1.pos(ii,:) - PC2.pos(jj,:)).^q; % normal cost
                end
            end
...
%         function obj = PCOT(PC1, PC2, xi) % version 2!
%             %PCOT Constructor using the 
%             %   Takes in two point clouds and the virtual cost parameter
%             
%             % copy in data
%             obj.PC1 = PC1;
...
%             % determine max amount of mass (s)
%             maxS = min(sum(abs(PC1.mass)), sum(abs(PC2.mass)));
% %             sRatio = sVec(sInd) * maxS;
%                 
%                 % set virtual point masses
% %                 m1 = [PC1.mass; sum(abs(PC2.mass)) - sRatio];
% %                 m2 = [PC2.mass; sum(abs(PC1.mass)) - sRatio];
%                 % Optimization
%                 totM1 = sum(abs(PC1.mass));
%                 totM2 = sum(abs(PC2.mass));
%                 % Optimization
%                 % Intreresting, this formualtion seemed that it could
%                 extend to optimize s too, but maybe not!
%                 cvx_begin % quiet
%                     variable s
%                     variable Tx(PC1.n+1, PC2.n+1)
%                     expression d
...
%                 
%                 sRatio = s./maxS;
%                 disp(['Found CVX Optimized sRatio = ' num2str(sRatio)]);
%                 
%                 obj.Tx = Tx;
%                 obj.T = Tx(1:PC1.n, 1:PC2.n);
%                 obj.sRatOpt = sRatio;
...
%             PCtm.pos = posTm;
%             PCtm.mass = TM;
%             PCtm.n = length(PCtm.mass);
%             
%             PCsm.pos = [posSm1; posSm2];
%             PCsm.mass = [Sm1; Sm2];
%             PCsm.n = length(PCsm.mass);
