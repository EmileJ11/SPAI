%% Partial Optimal Transport on Image Source Simulation
...
% Assumptions:
% - fixed order of image sources
% - pure distance atten. 1/r
% - wall reflection atten. -3 dB
% assign a single level per image source
...
rcvPos = ones(numRcvPos, 3) .* [3 4 1.8]; % starting position
...
for pairSep = 2:25
    ...
    for ii = 1:numPairs 
        ...
        scatter3(relSrcPos(1), relSrcPos(2), relSrcPos(3), 'og', 'filled');
        scatter3(0, 0, 0, '.k'); 
...
function PCall = combinePCs(pc1, pc2)
    PCall.n = pc1.n + pc2.n;
    PCall.pos = [pc1.pos; pc2.pos];
    PCall.mass = [pc1.mass; pc2.mass];
end
